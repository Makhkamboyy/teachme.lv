    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-lg-8" data-aos="fade-up">
          <div>
            <img :src="'http://localhost:8888' + getCourse.courseImage.contentUrl" class="img-fluid" alt="">
                  ...
                  <form method="post" @submit.prevent="submit" class="my-form">
                    ...
                    <div class="mb-3">
                      ...
                      <select class="form-select" aria-label="Default select example" v-model="schedule">
                        <option value="0">Choose the schedule:</option>
                        <option value="1">5.00 pm - 7.00 pm</option>
                        <option value="2">8.00 pm - 10.00 pm</option>
                        <option value="3">11.00 pm - 1.00 am</option>
                      </select>
                    </div>
                    <div class="mb-3">
...
export default {
  ...
  methods: {
    ...mapActions(['pushOrder', 'fetchCourse', 'fetchSchedules']),
    submit() {
      if (this.schedule) {
        if (this.schedule === "1") {
  ...
  computed: {
    ...
    ...mapGetters(['getCourse', 'getSchedules'])
  },
  mounted() {
    this
        .fetchCourse(this.$route.params.courseId)
        .then(() => {
          this.fullName = this.getCourse.price.trainer.name
          this.fee = '€' + this.getCourse.price.fee
          this.seat = this.getCourse.price.seat
        })
